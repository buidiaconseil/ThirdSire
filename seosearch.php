...
foreach ($listkey as &$value) {
   ...
   // echo $value;
    ...
    $finArray2=array_merge($finArray, $finNew[1]);
    $finArray=$finArray2;
    $url="https://fr.search.yahoo.com/sugg/gossip/gossip-fr-ura/?output=fxjson&command=".$trimed."&appid=fp&nresults=20&_=1559561814041";
    $finNew=json_decode(crawlUrl($url));
    $finArray2=array_merge($finArray, $finNew[1]);
    $finArray=$finArray2;
    ...
    $finNew=json_decode($molo);
    
    if(!strpos($molo,"error")>-1){
        $finArray2=array_merge($finArray, $finNew[1]);
        $finArray=$finArray2;
    }
