library(readr)
chiffres_cles_2017 <- read_delim("chiffres-cles-2017.csv", 
                                 ";", escape_double = FALSE, trim_ws = TRUE)
View(chiffres_cles_2017)
chiffres_cles_2015 <- read_delim("chiffres-cles-2015.csv", 
                                 ";", escape_double = FALSE, trim_ws = TRUE)
View(chiffres_cles_2015)

chiffre2017<-chiffres_cles_2017[,c(2,20,22:25)]
chiffre2016<-chiffres_cles_2017[,c(2,26,28:31)]
...
chiffre<-rbind(chiffre2013,chiffre2014,chiffre2015,chiffre2016,chiffre2017)
...
for(i in 1:nrow(chiffre2013Clean)){
  ...
  if(nrow(siren2014)==1 && nrow(siren2015)==1 && nrow(siren2016)==1 && nrow(siren2017)==1 ){
    racc<-c(siren,
...
colnames(chiffretraj)<- c("Siren","CA2013","CA2014","CA2015","CA2016","CA2017","Résultat2013","Résultat2014","Résultat2015","Résultat2016","Résultat2017","Effectif2013","Effectif2014","Effectif2015","Effectif2016","Effectif2017")


write_csv(chiffretraj,"chiffeAffaire2017.csv")

chiffretrajEnd<-chiffretraj
library(kml)
library(scales)
mtx <- matrix(t(1:5), ncol = 5) 
mtx.rep <- (apply(t(mtx), 1, rep,nrow(chiffretraj) )) 
time<-cbind(chiffretraj$Siren,mtx.rep)

data<-as.data.frame(cbind(chiffretraj$Siren,t(apply(chiffretraj[,2:6], 1, rescale))))
(ld2 <- clusterLongData(traj=data[,2:6],idAll=paste("I-",1:nrow(data),sep=""),time=(2:6)+0.5,varNames="R"))
kml(ld2,nbClusters=4)
chiffretrajEnd$clustersCA <- getClusters(ld2, 4)

data<-as.data.frame(cbind(chiffretraj$Siren,t(apply(chiffretraj[,7:11], 1, rescale))))
(ld2 <- clusterLongData(traj=data[,2:6],idAll=paste("I-",1:nrow(data),sep=""),time=(2:6)+0.5,varNames="R"))
kml(ld2,nbClusters=4)
chiffretrajEnd$clustersRES <- getClusters(ld2, 4)


data<-as.data.frame(cbind(chiffretraj$Siren,t(apply(chiffretraj[,12:16], 1, rescale))))
(ld2 <- clusterLongData(traj=data[,2:6],idAll=paste("I-",1:nrow(data),sep=""),time=(2:6)+0.5,varNames="R"))
kml(ld2,nbClusters=4)
...
for(i in 1:nrow(chiffretrajEnd)){
  ...
  listAct<-chiffres_cles_2017[chiffres_cles_2017$Siren==siren,6]
  caseAct<-unique(listAct[complete.cases(listAct),])
  if(nrow(caseAct)==1){
    chiffretrajEnd[i,20]=unique(listAct[complete.cases(listAct),])
  }
  listNom<-chiffres_cles_2017[chiffres_cles_2017$Siren==siren,1]
  caseNom<-unique(listNom[complete.cases(listNom),])
  if(nrow(caseNom)==1){
    chiffretrajEnd[i,21]=unique(listNom[complete.cases(listNom),])
  }
}
write_csv(chiffretrajEnd,"chiffeAffaireCluster2017.csv")
